...
class HWCBatchNorm2d(nn.Module):
...
class HWCTFSamePad2d(nn.Module):
...
# # Your constraint should look like this: zero threshold should be check again for the depthwise layer formation to follow . . . (error occured from that point to development of the constrainedconv 2d )
class ZeroThresholdConstraint:
    def __init__(self, threshold=2e-2): # Threshold for zeroing weights . should be make a application on those to process the intervals to work on . . . (this should increase for the torch application)
        self.threshold = threshold
    
    def __call__(self, w):
        # ✅ MUST return the modified tensor, not None!
        mask = torch.abs(w) >= self.threshold
        return w * mask.float()  # ✅ Return modified weights
        


class HWCConstrainedConv2d(HWCConv2d):
    """HWC Conv2d with weight constraints"""
    ...
    #         self.constraint(self.conv.weight)
    #     return super().forward(x)
    def forward(self, x):
        if self.constraint is not None:
            with torch.no_grad():
                self.conv.weight.data = self.constraint(self.conv.weight.data)  # ✅ USES return value!
        return super().forward(x)


class HWCSeparableConv2d(nn.Module):
    """Separable Conv2d for HWC format"""
    def __init__(self, in_channels, out_channels, kernel_size):
        super().__init__()
        self.samepad = HWCTFSamePad2d(kernel_size)
        self.depthwise = HWCConv2d(in_channels, in_channels, kernel_size,
                                   padding=0, groups=in_channels, bias=False)
        self.pointwise = HWCConv2d(in_channels, out_channels, 1, bias=False)
...
class FMRINet(nn.Module):
    ...
    def __init__(self, temporal_filters=8, num_classes=6, input_shape=(214, 277, 1),
        ...
        self.samepad1 = HWCTFSamePad2d((1, 60))
        self.conv1 = HWCConv2d(C, temporal_filters, (1, 60), padding=0, bias=False)
        ...
        self.depthwise_conv = HWCConstrainedConv2d(
        ...
        self.bn1 = HWCBatchNorm2d(temporal_filters * depth_multiplier)

        # --- AvgPool (1x15) VALID + Dropout ---
        self.avgpool1 = HWCAvgPool2d(kernel_size=(1, 15), stride=(1, 15))
        self.dropout_mid1 = nn.Dropout(dropout_mid)

        # --- SeparableConv2D SAME (1x8), no bias ---
        self.separable_conv = HWCSeparableConv2d(
            in_channels=temporal_filters * depth_multiplier,
            out_channels=64,
            kernel_size=(1, 8)
        )
        self.bn2 = HWCBatchNorm2d(64)

        # --- AvgPool (1x4) VALID + Dropout ---
        self.avgpool2 = HWCAvgPool2d(kernel_size=(1, 4), stride=(1, 4))
...
def build_fmri_net(temporal_filters=8, num_classes=6, input_shape=(214, 277, 1),
                   depth_multiplier=4, zero_thresh=7e-3, dropout_in=0.25,
